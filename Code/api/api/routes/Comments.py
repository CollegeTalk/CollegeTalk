...
class Comments(Resource):
    def get(self):
        ...
        comments = CommentModel.query.filter_by(post_id=post_id).order_by(
            CommentModel.helpful_answer.desc(), CommentModel.num_upvotes.desc()
        )
        ...
        for comment_data in result:
            comment_data["author_username"] = fetch_author_username(
    ...
    def post(self):
        ...
        try:
            data = request.json
            comment = CommentModel(data["author_id"], data["body"], data["post_id"])
            ...
            return jsonify(
                {
                    ...
                    "comments": [post_id for post_id in data["objs"]],
                }
            )
        except RuntimeError:
